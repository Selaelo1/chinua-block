...
const OnboardingPage = () => {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 'preferences':
        return <InvestmentPreferences onNext={handleNext} onBack={handleBack} />;
      case 'kyc':
        return <KYCVerification onBack={handleBack} onComplete={handleComplete} />;
      default:
        return null;
    }
  };
      ...
      <main className="max-w-3xl mx-auto px-4 py-12">
        <div className="bg-white rounded-lg shadow-lg">
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-gray-500 mt-1">Step {getCurrentStepIndex() + 1} of {steps.length}</p>
          </div>
                  ...
                  <React.Fragment key={step.id}>
                    <div className="flex flex-col items-center">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
                        getCurrentStepIndex() >= index ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600'
                      }`}>
                        {index + 1}
                      </div>
                    ...
                    {index < steps.length - 1 && (
                      <div className={`flex-1 h-0.5 mx-4 ${
                        getCurrentStepIndex() > index ? 'bg-blue-600' : 'bg-gray-200'
                      }`} />
                    )}
                  </React.Fragment>
                ))}
